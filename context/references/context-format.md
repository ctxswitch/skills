...
- **A definition opens with the term and nothing else.** `**Term**: definition`, with no qualifier, literal, or parenthetical between the name and the colon. A line that breaks the shape is read as a mention rather than a definition, and the term stops being owned anywhere. Where a literal identifies the term, it belongs inside the definition.
