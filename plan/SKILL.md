...
Read the code the change touches, plus its callers and its tests. Explore independent subsystems in parallel where the harness supports it.
...
- **Vocabulary** (`grill-me`) — Does every domain term mean what the existing code and docs already use it to mean? Does any phase contradict a documented decision without saying so? A plan that renames a domain concept in passing is changing the domain model without deciding to.
