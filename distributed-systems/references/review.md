...
Expect coverage of the **Minimum distributed test set** in [operations-testing.md](./operations-testing.md#minimum-distributed-test-set), plus the domain-specific cases in whichever reference covers the workflow under review.

Report a gap only where the untested case is material at this system's scale — an untested path whose failure is rare, low-impact, and cheaply recovered is a note, not a finding.
